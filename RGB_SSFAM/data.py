...
class SalObjDataset(data.Dataset):
    def __init__(
            self, image_root, gt_root, mask_root, gray_root, trainsize
    ):
        ...
        self.masks = [os.path.join(mask_root, f) for f in os.listdir(mask_root) if f.endswith('.jpg') or f.endswith('.png')]
        self.grays = [os.path.join(gray_root, f) for f in os.listdir(gray_root) if f.endswith('.jpg') or f.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        return image, gt, mask, gray, input_point, input_label
...
def get_loader(
        image_root, gt_root, mask_root, gray_root, batchsize, trainsize,
        shuffle=True, num_workers=0, pin_memory=True
):
    dataset = SalObjDataset(
        image_root, gt_root, mask_root, gray_root, trainsize,
    )
